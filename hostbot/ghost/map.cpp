...
void CMap :: Load( CConfig *CFG, string nCFGFile )
{
	...
	if( SFileOpenArchive( MapMPQFileName.c_str( ), 0, MPQ_OPEN_READ_ONLY, &MapMPQ ) )
	{
	...
	string mapname = MapMPQFileName;
	boost::algorithm::to_lower(mapname);

	bool YouTD108B = boost::contains(mapname, "youtd_v1.08b");
	bool GayBar214 = boost::contains(mapname, "Gay Bar 2.14");
		...
		else
		{
			...
			else
			{
				...
				if (MapMPQReady)
				{
					...
					if (YouTD108B)
					{
						CONSOLE_Print("DEBUG: youtd detected, using hardcoded CRC32/SHA1 values for 1.08b");

						MapCRC = BYTEARRAY{ (unsigned char)214, (unsigned char)191, (unsigned char)129, (unsigned char)228 };
						MapSHA1 = BYTEARRAY{
							(unsigned char)29,
							(unsigned char)114,
							(unsigned char)56,
	...
	if( !m_MapData.empty( ) )
	{
		if( MapMPQReady )
		{
			...
			if (SFileOpenFileEx(MapMPQ, "war3map.w3i", 0, &SubFile) || YouTD108B)
			{
				if (YouTD108B)
				{
					CONSOLE_Print("DEBUG: youtd detected, using hardcoded numplayers etc values for 1.08b");

					MapOptions = 96;
					MapNumPlayers = 9;
					MapNumTeams = 2;
					MapWidth = BYTEARRAY { (unsigned char)148, (unsigned char)0 };
					MapHeight = BYTEARRAY { (unsigned char)148, (unsigned char)0 };
					...
					Slots.push_back(CGameSlot(0, 255, SLOTSTATUS_OPEN, 0, 0, 0, SLOTRACE_HUMAN));
					Slots.push_back(CGameSlot(0, 255, SLOTSTATUS_OPEN, 0, 0, 1, SLOTRACE_ORC));
					Slots.push_back(CGameSlot(0, 255, SLOTSTATUS_OPEN, 0, 0, 2, SLOTRACE_UNDEAD));
					Slots.push_back(CGameSlot(0, 255, SLOTSTATUS_OPEN, 0, 0, 3, SLOTRACE_NIGHTELF));
					Slots.push_back(CGameSlot(0, 255, SLOTSTATUS_OPEN, 0, 0, 4, SLOTRACE_HUMAN));
					Slots.push_back(CGameSlot(0, 255, SLOTSTATUS_OPEN, 0, 0, 5, SLOTRACE_ORC));
					Slots.push_back(CGameSlot(0, 255, SLOTSTATUS_OPEN, 0, 0, 6, SLOTRACE_UNDEAD));
					Slots.push_back(CGameSlot(0, 255, SLOTSTATUS_OPEN, 0, 0, 7, SLOTRACE_NIGHTELF));
					Slots.push_back(CGameSlot(0, 255, SLOTSTATUS_OCCUPIED, 1, 1, 8, SLOTRACE_HUMAN));

					for (vector<CGameSlot> ::iterator i = Slots.begin(); i != Slots.end(); i++)
						CONSOLE_Print("[MAP] calculated map_slot = " + UTIL_ByteArrayToDecString((*i).GetByteArray()));
				}
				else
				{
					uint32_t FileLength = SFileGetFileSize(SubFile, NULL);
