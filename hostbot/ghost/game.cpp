...
bool CGame :: EventPlayerBotCommand( CGamePlayer *player, string command, string payload )
{
	...
	if( player->GetSpoofed( ) && ( AdminCheck || RootAdminCheck || IsOwner( User ) ) )
	{
		...
		if( !m_Locked || RootAdminCheck || IsOwner( User ) )
		{
			...
			******************/
			...
			if( Command == "fake" && !m_CountDownStarted )
			{
			...
			if( Command == "unpause" && m_FakePlayerPID != 255 && m_GameLoaded )
			{
				BYTEARRAY CRC;
				BYTEARRAY Action;
				Action.push_back( 2 );
			...
			//
			// !FROM XXX !p jest od tego
			//

			if( Command == "fromXXXXXXXXXX" )
			{
				string Froms;
			...
			if( Command == "l" || Command == "latency" )
			{
				if( Payload.empty( ) )
					SendAllChat( "Latency: " + UTIL_ToString( m_Latency ) );
				else
				{
					...
					{
						m_Latency = 20;
						SendAllChat( "Latency: 20" );
					}
					else if( m_Latency >= 500 )
					{
						m_Latency = 500;
						SendAllChat("Latency: 500");
					}
					else
						SendAllChat("Latency: " + UTIL_ToString(m_Latency));
				}
			...
			if ( ( Command == "start" || Command == "s" || Command == "go" ) && !m_CountDownStarted)
			{
	...
	*********************/
	...
	if (Command == "p" || Command == "f" || Command == "from" || Command == "ping" || Command == "check" || Command == "checkme" )
	{
		...
		{
			Pings += "(" + m_GHost->m_DBLocal->FromCheck(UTIL_ByteArrayToUInt32((*i)->GetExternalIP(), true)) + ") ";
			...
			if ((*i)->GetNumPings() > 0)
			{
				...
				// NIE MA KICKOWANIA CHYBA ZE JA TAK POWIEM
