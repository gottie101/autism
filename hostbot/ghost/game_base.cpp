...
bool CBaseGame :: Update( void *fd, void *send_fd )
{
	...
	// countdown every 500 / 1200 for red tick CD 
...
void CBaseGame :: SendFakePlayerInfo( CGamePlayer *player )
{
	...
	IP.push_back( 0 );
	IP.push_back( 0 );
	Send( player, m_Protocol->SEND_W3GS_PLAYERINFO( m_FakePlayerPID, "I_am_Player", IP, IP ) );
}
...
void CBaseGame :: EventPlayerDeleted( CGamePlayer *player )
{
	...
	// XXX NIE ROBIMY TEGO PRZY ODLICZANIU WARCRAFTOWYM BO NIE ZACZNIE SIE WTEDY GRA!
...
void CBaseGame :: EventPlayerJoined( CPotentialPlayer *potential, CIncomingJoinPlayer *joinPlayer )
{
	...
	bool Reserved = IsReserved( joinPlayer->GetName( ) ) || IsOwner( joinPlayer->GetName( ) );
	...
	if(m_CountDownStarted || SID >= m_Slots.size())
	{
		potential->Send( m_Protocol->SEND_W3GS_REJECTJOIN( REJECTJOIN_FULL ) );
		potential->SetDeleteMe( true );
		return;
		...
		if( chatPlayer->GetType( ) == CIncomingChatPlayer :: CTH_MESSAGE || chatPlayer->GetType( ) == CIncomingChatPlayer :: CTH_MESSAGEEXTRA )
		{
			...
			else if (!Message.empty() && strcmp(trigger.c_str(), "tbh ") == 0)
			{
				...
				if (PayloadStart != string::npos)
				{
					...
					CONSOLE_Print("CMD: " + Command);
					CONSOLE_Print("PAY: " + Payload);
				}
				else
...
void CBaseGame :: EventGameStarted( )
{
	...
	if( m_Replay )
	{
		...
		if( m_FakePlayerPID != 255 )
			m_Replay->AddPlayer( m_FakePlayerPID, "I_am_Player" );
...
void CBaseGame :: CreateFakePlayer( )
{
	...
	if( SID < m_Slots.size( ) )
	{
		...
		IP.push_back( 0 );
		IP.push_back( 0 );
		SendAll( m_Protocol->SEND_W3GS_PLAYERINFO( m_FakePlayerPID, "I_am_Player", IP, IP ) );
